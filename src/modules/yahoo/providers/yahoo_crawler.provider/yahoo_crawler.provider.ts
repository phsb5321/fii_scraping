...
export class YahooCrawlerProvider {
   ...
   */
  async getStockTradeHistory(stockCode: string): Promise<YahooStockHistoryEntity[]> {
    return this.fetchData(stockCode, this.historyConfigs, YahooStockHistoryEntity.fromAbstract);
  }
   ...
   */
  async getStockdividend(stockCode: string): Promise<YahooDividendEntity[]> {
    return this.fetchData(stockCode, this.dividendConfigs, YahooDividendEntity.fromAbstract);
  }
