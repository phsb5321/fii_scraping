...
export class YahooService {
  ...
  async updateStocks(stockCode: number): Promise<[YahooHistoryModelDB[], YahooDividendHistoryModelDB[]]> {
    try {
  ...
  async handleUpdateYahooStock(job: Job<{ stockCode: number }>): Promise<void> {
    try {
      ...
      this.logger.verbose(`Handled job ${job.id} for stock ${job.data.stockCode}`);
    } catch (error) {
      this.logger.error(`Error in job ${job.id} for stock ${job.data.stockCode}`, error.stack);
      throw error;
    }
  }
}
