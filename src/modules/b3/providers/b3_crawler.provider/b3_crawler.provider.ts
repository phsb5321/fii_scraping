...
export class B3CrawlerProvider {
  ...
  private buildURL(endpoint: string, options: QueryOptionsI): string {
    const base64Options = Buffer.from(JSON.stringify(options)).toString(
      "base64"
    );
  ...
  public async getStocks(): Promise<Stock[]> {
    const firstPage = await this.fetchStocks(
      DEFAULT_PAGINATION,
      this.b3Endpoints.list
    );
    ...
    const allStocks = [firstPage, ...remainingPages].flatMap(
      (page) => page.results
    );
  ...
  public async getStockDetails(codeCVM: string | string[]): Promise<Stock[]> {
    ...
    const stockDetails = await Promise.all(
      stocks.map((code) =>
        this.fetchStocks(
          { language: "pt-br", codeCVM: code },
          this.b3Endpoints.details
        )
      )
    );
