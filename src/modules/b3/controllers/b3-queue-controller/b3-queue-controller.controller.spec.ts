...
import { B3QueueController } from "@/modules/b3/controllers/b3-queue-controller/b3-queue-controller.controller";
...
describe("B3QueueController", () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: getQueueToken("stocks-queue"), useValue: mockQueue },
      ],
    }).compile();

    controller = module.get<B3QueueController>(B3QueueController);
    b3Service = module.get<B3Service>(B3Service);
    stocksQueue = module.get<Queue>(getQueueToken("stocks-queue"));
  });
  ...
  describe("enqueueScrapeAllStocksDetails", () => {
    it("should enqueue a scrape-all-stocks-details task", async () => {
      ...
      expect(stocksQueue.add).toHaveBeenCalledWith(
        "scrape-all-stocks-details",
        {}
      );
